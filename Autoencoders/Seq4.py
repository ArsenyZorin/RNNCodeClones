...
embeddings = tf.Variable(tf.random_uniform(weights.shape, -1.0, 1.0), dtype=tf.float32, trainable=False)
...
batches = np.array(model)
print(type(batches)) # numpy.ndarray
print(batches.shape) # (vocab_size, embedding_dim)
...
try:
    for batch in range(max_batches):
        ...
        if batch == 0 or batch % batches_in_epoch == 0:
            ...
            predict_ = sess.run([decoder_prediction, encoder_outputs], fd)
